// NOTE: QUADRATIC PROBING IS JUST SAME AS LINEAR PROBING
// IT IS USED TO SOLVE THE PROBLEM OF PRIMARY CLUSTERING IN LINEAR PROBING
// SO, IT PLACES ELEMENTS AT CERTAIN DISTANCE
// JUST REPLACE 'i' IN LINEAR WITH 'i*i' FOR QUADRATIC
...
int LinearProbe(int H[], int key){          // For Linear Probing
    ...
    int i=0;
    while(H[(index+i*i) % SIZE] != 0){                  // h'(x) = [(h(x)+f(i))%10]
...
int Search(int H[], int key){
    int index = Hash(key);
    int i=0;
    while(H[(index+i*i) % SIZE] != key){                // move on until you get your key
        i++;
        if(H[(index+i*i) % SIZE] == 0){                 // stop if you get an empty space
            return -1;
        }
    }
    return (index+i*i) % SIZE;                 // return the index where key is present
}
